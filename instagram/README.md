...
해당 코드를 실행했던 대상은 주로 사진 및 여행과 관련된 사람들 위주로 진행이 되었습니다.

`hash_tag = input("해시태그 입력 >> ")` 해당 코드를 사용해 해시 태그를 입력하면             
해당 해시태그를 입력한 사람들을 대상으로 코드가 실행되었습니다.   
...
좋아요+팔로우와 팔로우 코드에는 큰 차이가 없었습니다.                         

다만 좋아요+팔로우 코드로 진행할 경우 제 계정의 사진에 좋아요를 받을 확률만 높아지는 것이고            
저는 좋아요보다는 팔로우에 더 집중을 하고 있었기에         
최근에는 더 많은 팔로워 수를 늘리기 위해서 좋아요+팔로우 대신 팔로우만 하는 코드로 진행을 하게 되었습니다.           
